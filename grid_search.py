...
for _ in range(1):
    ...
    sn, model, depth = secure_random.choice([('500', 'resnet', '32')])
    chains = '10'
    ...
    os.system('python bayes_cnn.py -sn ' + sn + ' -type ' + types + ' -data ' + data + ' -model ' + model + ' -depth ' + depth + ' -batch ' + batch + ' -chains ' + chains \
            ...
            + ' > ./output/' + types + '_' + data + '_' + model + depth + '_sn_' + sn + '_batch_' + batch + '_chain_' + chains + '_lr_min_max_' + lr_min + '_' + lr_max + '_correction_' + correction + '_gamma_' + gamma + '_r_' + swap_rate + '_wd_' + window + '_scale_' + scale + '_pretrain_sn_300_v13_seed_' + seed)
