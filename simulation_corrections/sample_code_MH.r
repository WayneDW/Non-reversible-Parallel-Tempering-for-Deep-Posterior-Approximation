...
std = 1e-5
nosiy_f = function(x) return(f(x) + rnorm(1, 0, std))
...
x_high = 0
part1 = 100
thinning = 10 # setting lr=0.01 and thinning=1 easily leads to MAE 0.00638/ 0.01265/ 0.00701/ swaps avg: 29989
...
for (i in 1:part1) {
    ...
    for (j in 1:(total/part1)) {
        ...
        proposedx = rnorm(1, mean=x_low, sd=sd_proposal)

        A = exp(-nosiy_f(proposedx) + nosiy_f(x_low) - correction)
