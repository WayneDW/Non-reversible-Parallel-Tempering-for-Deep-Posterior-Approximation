...
sampler = PT_sampler(window=1, window_correction=0, std=0, num_samples=num_samples)

sampler = PT_sampler(window=100, window_correction=3.8, std=0, num_samples=num_samples)
...
MAE = np.mean(np.abs(gaussian_kde(real_samples)(x_ticks) - gaussian_kde(sampler.samples)(x_ticks)))
print(f'MAE {MAE: .2e}')
